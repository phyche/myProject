/**   
 * @Title: UserBusinessJMSQueueAppender.java 
 * @Package com.cyber.vo.logger 
 * @Description: 
 * @author cssuger@163.com   
 * @date 2016年5月25日 下午7:57:43 
 * @version V1.0   
 */
...
/**
 * @ClassName: UserBusinessJMSQueueAppender
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author cssuger@163.com
 * @date 2016年5月25日 下午7:57:43
 * 
 */
public class UserBusinessJMSQueueAppender extends
		JMSAppenderBase<ILoggingEvent> {
	...
	public void start() {
		Map<String, String> map = new PropertiesUtil(Constants.JNDI_PATH)
				.getReadMap();
		try {
			QueueConnectionFactory queueConnectionFactory = new ActiveMQConnectionFactory(
					(String) map.get("userName"),
					(String) map.get("password"),
					(String) map.get("providerUrl"));
			...
			Queue queue = this.queueSession.createQueue((String) map
					.get("businessQueue"));
	...
	public void append(ILoggingEvent event) {
		...
		try {
			...
			addError("Could not send message in JMSQueueAppender [" + this.name
					+ "].", e);
		}
	}
