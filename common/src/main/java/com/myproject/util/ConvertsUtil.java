 ...
 *
 */
public abstract class ConvertsUtil {
...
//            .getLogger(Converts.class);
    ...
    /**
     * 将整数转为16进制数后并以指定长度返回（当实际长度大于指定长度时只返回从末位开始指定长度的值）
     *
     * @param val
     *            int 待转换整数
     * @param len
...
//        if (str2 == null || "".equals(str2)) {
        ...
        if (len % 8 == 0) { // 字节8位
            for (int j = 0; j < len; j = j + 8) {
                ...
                // 2进制转10进制
                reStr += ConvertsUtil.longToHex(l, 2);
            }
            return reStr;
        }
        else {
    ...
    public static void main(String[] args) {
        String str = "898601106111080962253421776498";
//        System.out.println(asciiToHex(str));
        System.out.println(hexToBytes(str));
        System.out.println(bytesToString(hexToBytes(asciiToHex(str))));
        System.out.println(hexToAscii(bytesToString(hexToBytes(asciiToHex(str)))));
    }
}
